...
export default function EditBlogPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const existingImages = post.images.map(img => ({
          originalName: img.alt_text || 'Existing Image',
  ...
  const handleImageUpload = async (files: FileList | null, isThumbnail = false) => {
    ...
    // Add upload type and existing blog ID
    formData.append('uploadType', isThumbnail ? 'thumbnail' : 'detail');
    formData.append('blogId', postId); // Use existing blog ID
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        toast.error(errorData.error || 'เกิดข้อผิดพลาดในการแก้ไขบทความ');
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold mb-2">แก้ไขบทความ</h2>
          <p className="text-white/70">แก้ไขและอัปเดตบทความ</p>
        </div>
        <Link
          href="/dashboard/blog"
